...
public class Boss : MonoBehaviour
{
    ...
    [Header("##  DETECTION  ##")]
    private float detectionHeight = 0.5f;
    ...
    {
        // BLEED PARTICLES
        onHitBleed = transform.Find("BloodOnHit").GetComponent<ParticleSystem>();
        onDeathBleed = transform.Find("BloodOnDeath").GetComponent<ParticleSystem>();
        
        // RIGID LIMBS
        bossEye = transform.Find("body_bottom/body_up/head/BossEye").gameObject;
    ...
    {
        Vector3 direction = (player.transform.position - bossRigid.transform.position).normalized;
        direction.y = 0;
        StartCoroutine(Knocked());
        
        bossRigid.linearVelocity = new Vector3(-direction.x * knockbackSpeed, bossRigid.linearVelocity.y);
    }
    ...
    public void DamagePlayer()
    {
        if(PlayerInSight(direction) && !playerMovement.isDashing)
        {
            ...
            // KNOCKBACK PLAYER
            player.GetComponent<PlayerCombat>().rigidBody_up.linearVelocity = new Vector2(direction.x * 40, 0);
            playerRigid.linearVelocity = new Vector2(direction.x * 5, 0);
    ...
    IEnumerator FadeBoss()
    {
        ...
        if (rand % 2 == 0)
            rigidBody_up.linearVelocity = new Vector2(-direction.x * 30, 0);
        else
            rigidBody_bottom.linearVelocity = new Vector2(-direction.x * 30, 0);


        while (time < duration)
        {
            bossRigid.linearVelocity = new Vector2(0, 0);
            float alpha = Mathf.Lerp(1, 0, time / duration);

            Color newColorEnemy = boss_renderer.color;
            Color newColorEye = boss_renderer.color;
            newColorEnemy.a = alpha;
            newColorEye.a = alpha;
            boss_renderer.color = newColorEnemy;
    ...
    IEnumerator Attack()
    {
        ...
        transform.position = Vector2.Lerp(transform.position, new Vector2(transform.position.x + -transform.localScale.x * 2, transform.position.y), 0.9f);
